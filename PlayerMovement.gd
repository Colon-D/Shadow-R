...
const DECEL := 0.45
const DECEL_LIN := 0.8
const TURN := 0.8 # seems slow but that's what I calculated
const BRAKE := 0.4 # turn speed too for l or r brake
...
const MOVING_MULT := 2.0 # multiplier for TURN and BRAKE whilst moving
...
func _physics_process(delta: float) -> void:
	...
	if state == State.ACCELERATOR:
		...
		if state != State.SPINDASH:
			var forwards_accel := aim * Vector3(0, 0, max(Input.get_action_strength("accel"), 0)) * ACCEL
		...
		rotate_y(-input_dir * delta * rotate_vel)
		...
		if state != State.SWIM:
			...
			var rotate_by = brake_input * delta * brake_vel
			rotate_y(-rotate_by)
